...
#     jointLengths = [39.9035,39.6689,8.38147,28.5802,27.092,9.5622,39.9035,39.6689,8.38147,28.5802,27.092,9.5622,19.9754,20.7666]
    ...
    def error(self, x, prev_y, y, generated):
        ...
        if (self.foot_slide_weight > 0 or self.BALL_DIMENSION > 0):
            if self.IS_PREDICTION_MODEL:
                motion_prev = motion_prev[:,:self.Y_DIMENSION]
                motion_g = motion_g[:,:,:self.Y_DIMENSION]
                motion_y = motion_y[:,:,:self.Y_DIMENSION]
...
#         return loss_list[0], loss_list[1], loss_list[2]
    ...
    def motion_mse_loss(self, x, y, output):
        ...
        predictionStart = self.Y_DIMENSION

        output_root = tf.slice(output, [0, 0, rootStart], [-1, -1, self.ROOT_DIMENSION])
        output_pose = tf.slice(output, [0, 0, poseStart], [-1, -1, self.Y_DIMENSION - self.ROOT_DIMENSION])
        y_root = tf.slice(y, [0, 0, rootStart], [-1, -1, self.ROOT_DIMENSION])
        y_pose = tf.slice(y, [0, 0, poseStart], [-1, -1, self.Y_DIMENSION - self.ROOT_DIMENSION])
        if self.IS_PREDICTION_MODEL:
            output_prediction = tf.slice(output, [0, 0, predictionStart], [-1, -1, -1])
            y_prediction = tf.slice(y, [0, 0, predictionStart], [-1, -1, -1])
        ...
        if (self.activation_weight > 0):
            ...
            if (self.pose_joint_weights == None):
                ...
                if self.IS_PREDICTION_MODEL:
                    loss_pose += 0.3 * tf.reduce_mean(tf.square(output_prediction - y_prediction))
            else:
...
#                 f_contact = tf.clip_by_value(f_contact - 0.5, 0, 0.5)
    ...
    def load_normal_data(self, folder):
        xMean, xStd = loadNormalData("../motions/%s/data/xNormal.dat"%(folder))
        yMean, yStd = loadNormalData("../motions/%s/data/yNormal.dat"%(folder))
...
class RNNModel(object):
    def __init__(self, config, batchSize, stepSize, lr=0.0001):
        ...
        with config.scope():
            ...
            if self.config.IS_PREDICTION_MODEL:
                self.prev_y = tf.placeholder(tf.float32, [batchSize, config.Y_DIMENSION + config.X_DIMENSION], name="prev_y")
                self.y = tf.placeholder(tf.float32, [batchSize, stepSize, config.Y_DIMENSION + config.X_DIMENSION], name="y")
            else:
...
#         stacked_lstm = tf.contrib.rnn.MultiRNNCell([c.rnn_cell() for _ in range(c.NUM_OF_LAYERS)])
        ...
        hiddens = []
        if self.config.IS_PREDICTION_MODEL:
            output_w = tf.get_variable("output_w", [c.RNN_SIZE, c.X_DIMENSION + c.Y_DIMENSION], dtype=tf.float32)
            output_b = tf.get_variable("output_b", [c.X_DIMENSION + c.Y_DIMENSION], dtype=tf.float32)
        else:
...
#             cInput = inputs[:,i]
            if (c.use_input_layer):
                ...
                if (c.POSE_CONTAINED_AS_INPUT):
                    ...
                    if self.config.IS_PREDICTION_MODEL:
                        cInput = tf.concat([c.drop_out(inputs[:,i], c.INPUT_KEEP_PROB), output[:,:-c.X_DIMENSION]], 1)
